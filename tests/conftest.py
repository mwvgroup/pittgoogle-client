...
@pytest.fixture
def sample_alerts_rapid() -> list[load_data.TestAlert]:
    """List of all RAPID sample alerts."""
    survey = "rapid"
    alert_paths = _get_sample_alert_paths(survey)
    alerts = []
    for alert_path in alert_paths:
        # Expecting alert_path names like "rapid.v01_00.avro".
        schema_fname = alert_path.with_suffix(".alert.avsc").name
        schema_version = alert_path.suffixes[0].strip(".")
        major, minor = schema_version.strip("v").split("_")
        schema_path = SCHEMAS_DIR / survey / major / minor / schema_fname
...
@pytest.fixture
def sample_alert_rapid_latest(sample_alerts_rapid) -> load_data.TestAlert:
    latest_version = sorted(sa.schema_version for sa in sample_alerts_rapid)[-1]
    for sample_alert in sample_alerts_rapid:
        if sample_alert.schema_version == latest_version:
            return sample_alert
