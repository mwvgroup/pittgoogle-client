...
- `Schema`:
    - Dedicated classes for lite schemas, specifically `ZtfliteSchema`.
    - Schema map for the ZTF lite schema.
- `Registry`
    - Support for `ztf.lite` schema

### Changed

- Rename `Alert` method `_add_id_attributes()` -> `_add_attributes()` to reflect that more properties are added.
- `Registry`:
    - `_get()` and `_get_class()` methods now handle cases where `schema_name` includes punctuation (e.g., `ztf.lite`)
